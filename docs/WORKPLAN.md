...
- [x] A4 AppRuntime extraction: перенести app-level зависимости из `GameWindow`/entrypoints в ручную композицию: settings, paths, event bus, audio, content registries, texture pack service, settings store.
  ...
  - [x] A4.4 Extract texture pack service ownership; wired through `AppRuntime.texture_packs` and shared by A7 `ApplyResourcePackUseCase`.
- [x] A5 WorldRuntime extraction: перенести world-level зависимости: active world storage, block registry, generation/streaming, player state, entity world, simulation systems, renderer facade/port.
...
- [x] A8 Split renderer/world ownership boundaries gradually: storage/generator/streamer/fluid/lighting/registry уходят в runtime/simulation; renderer остаётся GPU scene adapter.
- [x] A9 Add isolated tests use cases/systems: resource pack apply, player movement, fluid step, mob spawning, generation pipeline без Pyglet/OpenGL.

## Immediate Next Step

Последний завершённый шаг: Phase A closeout marked completed after A4.4 texture-pack service ownership moved to `AppRuntime` and A9 isolated test coverage was audited.

Следующий кодовый шаг: start the next gameplay phase from this stabilized baseline.

1. Pick the next gameplay focus: player feel, water, Twilight-like generation, 3D player, UI polish, mobs, network, or resource packs.
2. Keep new systems behind runtime/use-case boundaries from `docs/ARCHITECTURE.md`.
3. Add isolated subsystem tests before GPU/window integration.
