...
- [ ] **Игрок не может плавать** — нет логики buoyancy в PlayerController
- [ ] **Вода не течёт** — нет flow physics (source → flowing → decay)
- [ ] **Нельзя ломать блоки сквозь воду** — raycast блокируется водой

**Файлы:** `engine/physics/player.py`, `engine/generation/streaming.py`, `render/world_scene.py`

### 0.2 Mob AI & Physics
- [ ] **Мобы застревают в воде** — нет water navigation для AI
- [ ] **Мобы спавнятся внутри блоков** — нет проверки solid blocks при спавне
- [ ] **AI не обходит препятствия** — pathfinding отсутствует или примитивный
- [ ] **Зомби бьёт игрока сквозь высоту** — нет проверки Y-distance для атак

### 0.3 Combat & Animation
- [ ] **Анимация удара зомби** — каждый удар должен сопровождаться анимацией
- [ ] **Высотная проверка атаки** — зомби не может ударить если игрок >2 блока выше
